...
이 기능은 지금 화면에 보이는 거리뷰만 잠깐 사용해 하늘만 밤으로 바꾸고 달을 올리며, 원본 이미지는 서버에 저장하지 않습니다. 완성된 이미지는 `이미지 저장`으로 받을 수 있습니다. 기본 이미지 모델은 Gemini 3.1 Flash Image(Nano Banana 2)입니다.
