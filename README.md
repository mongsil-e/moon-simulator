...
`밤 장면 만들기`를 누르면 휴대폰이나 PC 브라우저에 보이는 거리뷰 영역과 계산된 달 위치 안내를 이미지로 담아 Gemini에 전달합니다. 서버 컴퓨터의 모니터를 캡처하지 않으므로 같은 와이파이에서 휴대폰으로 접속해도 사용할 수 있습니다. Gemini는 안내 달의 중심과 표시 크기를 유지하면서 달 표면, 대기광, 주변 달빛까지 실제 밤 사진처럼 자연스럽게 다시 만듭니다. 달은 알아보기 쉽도록 엄밀한 각지름의 3배로 표시합니다. 원본 캡처는 서버에 저장하지 않으며, 완성된 이미지는 `이미지 저장`으로 받을 수 있습니다. 기본 이미지 모델은 Gemini 3.1 Flash Image(Nano Banana 2)입니다.
